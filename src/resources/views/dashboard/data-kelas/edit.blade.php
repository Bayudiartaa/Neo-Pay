                  ...
                  <div class="card-body">
                       <div class="card-title">{{ __('Edit Kelas') }}</div>
                        ...
                        <form method="post" action="{{ url('/dashboard/data-kelas', $edit->id) }}">
                           ...
                           <div class="form-group">
                              <select name="id_kompetensi_keahlian" class="form-control custom-select @error('id_kompetensi_keahlian') is-invalid @enderror">							
                                 @foreach($skill as $row)   
                                       <option value="{{ $row->id }}" {{ $edit->id_kompetensi_keahlian == $row->id ? 'selected' : '' }}>{{ $row->name }}</option>											
                                 @endforeach
                              </select>
                           </div>
